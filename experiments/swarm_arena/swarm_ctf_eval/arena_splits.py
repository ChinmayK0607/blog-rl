...
FROZEN_EVAL_CASES: tuple[tuple[int, int, str], ...] = tuple(
    (seed, 12 + index // 20, _STYLES[index % len(_STYLES)])
    for index, seed in enumerate(_FROZEN_SEEDS)
)

FROZEN_EVAL_SEEDS = frozenset(seed for seed, _, _ in FROZEN_EVAL_CASES)
FROZEN_EVAL_MANIFEST_SHA256 = hashlib.sha256(
    json.dumps(FROZEN_EVAL_CASES, separators=(",", ":")).encode()
).hexdigest()
