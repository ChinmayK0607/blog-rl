...
for step in "$@"; do
  ...
  "$uv_bin" run --project "$project_root" --no-sync python -m scripts.score_regressions \
...
"$uv_bin" run --project "$project_root" --no-sync python -m swarm_ctf_eval.warm_start_selection_v3 \
